...
Vue.component('ticket-form', {
    ...
    template:
        ...
        '<input type="button" value="Add ticket" @click="save" />' +
        '</div>',
    methods: {
        save: function () {
            var ticket = { text: this.text };
...
Vue.component('tickets-list', {
    ...
    template: '<div style="position: relative; width: 300px;">' +
        ...
        '<ticket-row v-for="ticket in tickets" :key="ticket.id" :ticket="ticket" :editMethod="editMethod" :tickets="tickets"/></div>' +
        '</div>',
    created: function () {
